...
typedef struct cmd_spec {
    const char *name;
    const char *version;    /* semver string, e.g. "1.0.0" (NULL → "1.0.0") */
    const char *summary;
...
static cmd_spec_t * const COMMAND_REGISTRY[] = {
    ...
    /* shell */
    &spec_pwd, &spec_cd, &spec_env, &spec_export, &spec_unset, &spec_type, &spec_echo, &spec_hello,
    /* search */
    &spec_rg,
    /* editing */
